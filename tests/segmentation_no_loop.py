...
for j in range(3, 61):
    ...
    for i in range(num_file):
        ...
        if not os.path.exists(add1 + '/rope_no_loop_seg/' + add2):
            os.mkdir(add1 + '/rope_no_loop_seg/' + add2)
        ...
        if not os.path.exists(add1 + '/rope_no_loop_green_mask/' + add2 + add3):
            continue

        # Read the image and transform it to HSV color space
        img = cv2.imread(add1 + '/rope_no_loop_green_mask/' + add2 + add3)
        ...
        cv2.imwrite(add1 + '/rope_no_loop_seg/' + add2 + add3, res_green_light)
