...
class CAE(nn.Module):
    def __init__(self, latent_state_dim=80, latent_act_dim=80):
        ...
        # state
        self.conv_layers = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1),  
                                         nn.ReLU(),
        ...
        self.dconv_layers = nn.Sequential(nn.ConvTranspose2d(128, 128, 3, stride=2, padding=1),
                                          ...
                                          nn.ReLU(),                                         
                                          nn.ConvTranspose2d(32, 3, 2, stride=2, padding=2),
                                          nn.Sigmoid())
        # action
...
class SysDynamics(nn.Module):
    def __init__(self, latent_state_dim=80, latent_act_dim=80):
        ...
        self.conv_layers_matrix = nn.Sequential(nn.Conv2d(3, 32, 3, padding=1),  
                                            nn.ReLU(),
...
def constraint_loss(steps, idx, trainset, U_latent, L):
    loss = 0
    data = trainset.__getitem__(idx).float().to(device).view(-1, 3, 50, 50)
    embed_state = model.encoder(data).detach().cpu().numpy()
    for i in range(steps):
        step = i + 1  
        data_next = trainset.__getitem__(idx+step).float().to(device).view(-1, 3, 50, 50)
...
def train_new(epoch, recon_model, dyn_model, epoch_thres=500):
    if epoch < epoch_thres:
        ...
        for batch_idx, batch_data in enumerate(trainloader):
            ...
            # plt.show()        
            img_cur = img_cur.float().to(device).view(-1, 3, 50, 50)
            # action
            act = batch_data['resz_action_cur']
            act = act.float().to(device).view(-1, 4)
            # image after action
            img_post = batch_data['image_bi_post']
            img_post = img_post.float().to(device).view(-1, 3, 50, 50)   
            # comparison = torch.cat([batch_data['image_bi_cur'][:8],                 # current image
            #                         batch_data['image_bi_post'][:8]])      # reconstruction of current image                                                 
            # save_image(comparison.cpu(),
            #         '/home/zwenbo/Desktop/test_img.png', nrow=8)                   
            # optimization
            ...
            if batch_idx == 0:
                ...
                comparison = torch.cat([batch_data['image_bi_cur'][:n],                 # current image
                                    recon_img_cur.view(-1, 3, 50, 50).cpu()[:n]])      # reconstruction of current image                                                 
                save_image(comparison.cpu(),
        ...
        for batch_idx, batch_data in enumerate(trainloader):
            ...
            if batch_idx == 0:
                ...
                comparison = torch.cat([batch_data['image_bi_cur'][:n],                 # current image
                                    ...
                                    pred_img_post.view(-1, 3, 50, 50).cpu()[:n]])     # prediction of post image
                save_image(comparison.cpu(),
                        './result/{}/reconstruction_train/reconstruct_epoch_{}.png'.format(folder_name, epoch), nrow=n)      
                plot_sample(batch_data['image_bi_cur'][:n].detach().cpu().numpy(), 
                            batch_data['image_bi_post'][:n].detach().cpu().numpy(), 
...
parser.add_argument('--folder-name', default='test_RGB', 
...
resz_act_path = './rope_dataset/rope_clean_all_resize50/rope_clean_all_size50.npy'
