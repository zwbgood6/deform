...
class CAE(nn.Module):
    def __init__(self, latent_state_dim=500, latent_act_dim=100):
        ...
        # state
        self.conv_layers = nn.Sequential(nn.Conv2d(1, 8, 3, padding=1), # TODO: kernel size 3
                                         nn.ReLU(),
                                         nn.MaxPool2d(3, stride=2),
                                         nn.Conv2d(8, 16, 3, padding=1), # TODO: 64 channels, increase number of layers
                                         nn.ReLU(),
                                         nn.MaxPool2d(3, stride=2),
                                         ...
                                         nn.ReLU(),
                                         nn.MaxPool2d(3, stride=2))  # TODO: four five layer
...
def get_U(action):         
    ...
    action = model.encoder_act(action).detach().cpu().numpy()
...
def get_G(image_pre, image_post):
    ...
    latent_post = model.encoder(image_pre).detach().cpu().numpy().reshape(n,-1)
...
def train_new(epoch):
    ...
    for batch_idx, batch_data in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        # model
        recon_img_pre, recon_act = model(img_pre, act)
        # loss
        loss_img = loss_function_img(recon_img_pre, img_pre)
        loss_act = loss_function_act(recon_act, act)
        loss_latent = loss_function_latent(img_pre, img_post, act)
    ...
    return train_loss/n, img_loss/n, act_loss/n, latent_loss/n
        ...
        for batch_idx, batch_data in enumerate(testloader):
            ...
            if batch_idx == 0:
                ...
                comparison = torch.cat([batch_data['image_pre'][:n],
                                      recon_img_pre.view(-1, 1, 50, 50)[:n]])
                save_image(comparison.cpu(),
                         './result/{}/reconstruction/reconstruct_epoch_'.format(folder_name) + str(epoch) + '.png', nrow=n)                                         
...
action_path = './rope_dataset/rope_all/actions.npy'
...
for epoch in range(1, epochs+1):
    train_loss, img_loss, act_loss, latent_loss = train_new(epoch)
...
plot_test_loss('./result/{}/img_loss_epoch{}.npy'.format(folder_name, epochs), folder_name)
plot_test_loss('./result/{}/act_loss_epoch{}.npy'.format(folder_name, epochs), folder_name)
plot_test_loss('./result/{}/latent_loss_epoch{}.npy'.format(folder_name, epochs), folder_name)
