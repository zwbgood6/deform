...
class CAE(nn.Module):
    ...
    def forward(self, x_cur, x_post):
        ...
        return g_cur, g_post, self.decoder(g_cur)
...
class SysDynamics(nn.Module):
    ...
    def forward(self, x_cur):
        ...
        return self.encoder_matrix(x_cur)
...
def train_new(epoch, recon_model, dyn_model, epoch_thres=500):
    if epoch < epoch_thres:
        ...
        for batch_idx, batch_data in enumerate(trainloader):
            ...
            # model
            latent_img_cur, latent_img_post, recon_img_cur = recon_model(img_cur, img_post)
            # prediction
...
for epoch in range(init_epoch, epochs+1):                                                    
    train_loss, train_img_loss, train_act_loss, train_latent_loss, train_pred_loss = train_new(epoch, recon_model, dyn_model, epoch_thres=int(epochs))
