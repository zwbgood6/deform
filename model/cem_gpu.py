...
def sample_action(I, mean=None, cov=None):
    ...
    action = torch.tensor([0]*4, dtype=torch.float)#.cuda() 
    multiplier = torch.tensor([50, 50, 2*math.pi, 0.14])#.cuda() 
    addition = torch.tensor([0, 0, 0, 0.01])#.cuda() 
    thres = 0.9
    ...
    while I[0][0][torch.floor(action[0]).type(torch.LongTensor)][torch.floor(action[1]).type(torch.LongTensor)] != 1.: # < thres means grasping black region, 255 is max pixtel intensity for white color 
        if ((mean is None) and (cov is None)):
            ...
            #cov = add_eye(cov)
            cov = add_eye(cov)
            action = MultivariateNormal(mean, cov).sample()#.cuda()  # dont need to use multiplication and addition
...
def generate_next_pred_state_in_n_step(recon_model, dyn_model, img_initial, N, H, mean=None, cov=None):
    ...
    for n in range(N):
        ...
        for h in range(H):
            ...
            img_tmp = generate_next_pred_state(recon_model, dyn_model, img_tmp, action.cuda())
...
def get_image(i):
    ...
    return img.reshape((-1, 1, 50, 50)).cuda().type(torch.float)
